...
public class InsuranceCalcBrazilService implements InsuranceCalcService {
    private static final Long BASIS_ROBBERY_INSURANCE_COST = 300L;
    private static final Long BASIS_HEALTH_INSURANCE_COST = 800L;
    ...
    public BigDecimal getCostInsurance(BigDecimal coverageAmountInsurance, InsuranceType insuranceType) {
        if (InsuranceType.ROBBERY.equals(insuranceType)) {
            return calculateCostInsurance(coverageAmountInsurance,
                    coefficientRobbery, BASIS_ROBBERY_INSURANCE_COST);
        }
        return calculateCostInsurance(coverageAmountInsurance,
                coefficientHealth, BASIS_HEALTH_INSURANCE_COST);
    }
}
